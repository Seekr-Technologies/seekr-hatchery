...
def dockerfile_path(base: Path, backend: agent.AgentBackend) -> Path:
    ...
    Files are named ``Dockerfile.<agent>`` (e.g. ``Dockerfile.claude``).
    """
...
def ensure_dockerfile(repo: Path, backend: agent.AgentBackend = agent.CLAUDE) -> bool:
...
def _run_container(
    ...
    agent_cmd: list[str],
    backend: agent.AgentBackend = agent.CLAUDE,
    dind: bool = False,
    runtime: Runtime = Runtime.DOCKER,
    ...
    *proxy_token* is a stable per-task UUID used as the API key env var inside
    ...
    # Shadow agent-specific tmpfs paths (e.g. ~/.claude/backups for Claude).
...
def launch_docker(
    ...
    *agent_cmd* must be the full command already built for Docker mode
    ...
    # (e.g. Claude seeds trusted folder + proxy token approval in claude.json).
...
def resolve_runtime(
    repo: Path, worktree: Path, no_docker: bool, backend: agent.AgentBackend = agent.CLAUDE
) -> Runtime | None:
    ...
    Checks for the agent-specific Dockerfile (e.g. ``Dockerfile.claude``).
    """
    if no_docker:
