...
class UserConfig:
    ...
    def resolve_backend(self, agent_name: str | None) -> agent.AgentBackend:
        ...
        3. Zero binaries on ``$PATH`` → fall back to :data:`agent.CLAUDE` silently
        ...
        all_backends: list[agent.AgentBackend] = [agent.CLAUDE, agent.CODEX]
        ...
        if len(detected) == 0:
            logger.debug("No agent binary found on $PATH — defaulting to claude (Docker workflow)")
            return agent.CLAUDE
