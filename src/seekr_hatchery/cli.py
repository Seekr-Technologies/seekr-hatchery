...
def _post_exit_check(
    ...
    if choice == "w":
        ...
        if not session_id:
            ...
            return
        backend = agent.from_kind(meta.get("agent", "CLAUDE"))
...
def cli(log_level: str, log_file: str | None) -> None:
    """AI coding agent task orchestration (Claude Code, OpenAI Codex)."""
...
def cmd_status(name: str) -> None:
    ...
    click.echo(click.style("Agent:    ", bold=True) + meta.get("agent", "CLAUDE").lower())
