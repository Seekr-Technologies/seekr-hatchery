...
class TestHelp:
    def test_help_lists_commands(self):
        ...
        assert result.exit_code == 0
        for cmd in ("new", "resume", "done", "archive", "delete", "list", "status", "config"):
...
class TestCliNoWorktree:
    ...
    def test_no_worktree_with_dockerfile_uses_launch_docker_no_worktree(self):
        ...
        with ExitStack() as stack:
            ...
            # Override _launch_claude_new to NOT be mocked so Docker path is exercised
            mock_launch.side_effect = None  # still mocked; just verify docker call
            result = runner.invoke(cli, ["new", "my-task", "--no-worktree"])

        # _launch_claude_new is still mocked, so we just verify the flag propagates
        assert result.exit_code == 0
