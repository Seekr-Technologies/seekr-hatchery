...
class _FakeBackend:
    def container_env(self, endpoint, proxy_token: str, proxy_port: int) -> dict[str, str]:
        return {
            f"{endpoint.key.upper()}_API_KEY": proxy_token,
            f"{endpoint.key.upper()}_BASE_URL": f"http://host.docker.internal:{proxy_port}",
        }
