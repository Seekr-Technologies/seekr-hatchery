...
class TestMigrate:
    def test_v0_migrates_to_v1(self):
        assert user_config._migrate({"default_agent": "CLAUDE"}) == {
            "schema_version": "1",
...
class TestUserConfigLoad:
    ...
    def test_v0_file_is_migrated(self, tmp_path):
        path = tmp_path / "config.json"
        path.write_text(json.dumps({"default_agent": "CLAUDE"}))
        cfg = user_config.UserConfig.load(path)
        assert cfg.schema_version == "1"
...
class TestValidateConfigFile:
    ...
    def test_v0_file_passes_after_migration(self, tmp_path):
        path = tmp_path / "config.json"
        path.write_text('{"default_agent": "CLAUDE"}')
        assert user_config.validate_config_file(path) is None
...
class TestResolveBackendExplicit:
    def test_known_backends(self, tmp_path):
        cfg = user_config.UserConfig.load(tmp_path / "config.json")
        assert cfg.resolve_backend("claude") is agent.CLAUDE
        assert cfg.resolve_backend("codex") is agent.CODEX
        assert cfg.resolve_backend("CLAUDE") is agent.CLAUDE  # case-insensitive
...
class TestResolveBackendAutoDetect:
    def test_single_detected_returns_it(self, tmp_path, monkeypatch):
        monkeypatch.setattr("seekr_hatchery.user_config._detect_installed", lambda _: [agent.CLAUDE])
        cfg = user_config.UserConfig.load(tmp_path / "config.json")
        assert cfg.resolve_backend(None) is agent.CLAUDE

    def test_zero_detected_returns_claude_without_saving(self, tmp_path, monkeypatch):
        path = tmp_path / "config.json"
        ...
        assert result is agent.CLAUDE
        assert not path.exists()

    def test_multiple_detected_uses_saved_default(self, tmp_path, monkeypatch):
        path = tmp_path / "config.json"
        monkeypatch.setattr("seekr_hatchery.user_config._detect_installed", lambda _: [agent.CLAUDE, agent.CODEX])
        cfg = user_config.UserConfig.load(path)
        cfg.set_default_agent("CODEX")
        cfg.save()
        assert user_config.UserConfig.load(path).resolve_backend(None) is agent.CODEX

    def test_multiple_detected_no_default_prompts_and_saves(self, tmp_path, monkeypatch):
        path = tmp_path / "config.json"
        monkeypatch.setattr("seekr_hatchery.user_config._detect_installed", lambda _: [agent.CLAUDE, agent.CODEX])
        monkeypatch.setattr("builtins.input", lambda _: "2")  # choose codex
        cfg = user_config.UserConfig.load(path)
        result = cfg.resolve_backend(None)
        assert result is agent.CODEX
        assert user_config.UserConfig.load(path).default_agent == agent.CODEX.kind

    def test_multiple_detected_invalid_saved_default_reprompts(self, tmp_path, monkeypatch):
        path = tmp_path / "config.json"
        monkeypatch.setattr("seekr_hatchery.user_config._detect_installed", lambda _: [agent.CLAUDE, agent.CODEX])
        cfg = user_config.UserConfig.load(path)
        cfg.set_default_agent("obsolete-agent")
        cfg.save()
        monkeypatch.setattr("builtins.input", lambda _: "1")  # choose claude
        result = user_config.UserConfig.load(path).resolve_backend(None)
        assert result is agent.CLAUDE
        assert user_config.UserConfig.load(path).default_agent == agent.CLAUDE.kind


# ---------------------------------------------------------------------------
# _prompt_and_save — retry loop (unique behavior not covered by resolve_backend tests)
# ---------------------------------------------------------------------------
...
class TestPromptAndSave:
    def test_invalid_then_valid_input(self, tmp_path, monkeypatch):
        ...
        cfg = user_config.UserConfig.load(tmp_path / "config.json")
        assert cfg._prompt_and_save([agent.CLAUDE, agent.CODEX]) is agent.CLAUDE
